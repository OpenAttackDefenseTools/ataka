...
from ataka.common.database.models import Flag, Execution, Target
        ...
        async with get_channel() as channel:
            ...
            async with database.get_session() as session:
                while True:
                    ...
                    queue_init = select(Flag).where(
                        Flag.status.in_({FlagStatus.PENDING, FlagStatus.ERROR})
                    )
                    init_list = list((await session.execute(queue_init)).scalars())

                    submit_list = [
                        FlagMessage(flag.id, flag.flag)
                        for flag in init_list
                        if flag.status == FlagStatus.PENDING
                    ]
                    resubmit_list = [
                        FlagMessage(flag.id, flag.flag)
                        for flag in init_list
                        if flag.status == FlagStatus.ERROR
                    ]
                    dupe_list = []
                    try:
                        async for message in flag_queue.wait_for_messages(
                            timeout=ratelimit
                        ):
                            ...
                            check_duplicates = (
                                ...
                                .where(
                                    Flag.status.in_(DuplicatesDontResubmitFlagStatus)
                                )
                                .limit(1)
                            )
                            duplicate = (
                                (await session.execute(check_duplicates))
                                .scalars()
                                .first()
                            )
                            if duplicate is not None:
                                ...
                                self._flag_cache[flag] = (
                                    FlagStatus.DUPLICATE_NOT_SUBMITTED
                                )
                            else:
                    ...
                    if len(submit_list) < batchsize and len(resubmit_list) > 0:
                        resubmit_amount = min(
                            batchsize - len(submit_list), len(resubmit_list)
                        )
                        print(
                            f"Got leftover capacity, resubmitting {resubmit_amount} errored flags "
                    ...
                    if len(submit_list) > 0:
                        ...
                        print(
                            f"Submitting {len(submit_list)} flags, {diff:.2f}s since last time"
                            + (
                                f" (sleeping {ratelimit-diff:.2f})"
                                if diff < ratelimit
                                else ""
                            )
                        )
                        ...
                        statuslist = self._ctf.submit_flags(
                            [flag.flag for flag in result]
                        )
        ...
        async with get_channel() as channel:
            ...
            async with database.get_session() as session:
                async for message in output_queue.wait_for_messages():
                    service = (
                        (
                            await session.execute(
                                ...
                                .where(
                                    Target.id
                                    == select(Execution.target_id).where(
                    ...
                    regex, group = self._ctf.get_flag_regex(service)
                    submissions = []
                    duplicates = []
                    for match in re.finditer(regex, message.output):
                        ...
                        if (
                            flag in self._flag_cache
                            and self._flag_cache[flag]
                            in DuplicatesDontResubmitFlagStatus
                        ):
                    ...
                    if len(submissions) > 0:
                        await asyncio.gather(
                            *[
                                flag_queue.send_message(
                                    FlagMessage(flag_id=f.id, flag=f.flag)
                                )
